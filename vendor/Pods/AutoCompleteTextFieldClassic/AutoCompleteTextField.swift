...
open class AutoCompleteTableRowView:NSTableRowView{
...
open class AutoCompleteTextField:NSTextField{
    ...
    open func tableViewSelectionDidChange(_ notification: Notification) {
        ...
        if selectedRow >= 0 && selectedRow < matchCount{
            ...
            if self.tableViewDelegate!.responds(to: Selector("didSelectItem:")){
...
//                self.insert(self)
    ...
    func insert(_ sender:AnyObject){
        ...
        if selectedRow >= 0 && selectedRow < matchCount{
            //self.stringValue = self.matches![selectedRow]
            if self.tableViewDelegate!.responds(to: Selector("didSelectItem:selectedItem:")){
                self.tableViewDelegate!.didSelectItem!(self.stringValue)
            }
        }
    ...
    func completionsForPartialWordRange(_ charRange: NSRange, indexOfSelectedItem index: Int) ->[String]{
        if self.tableViewDelegate!.responds(to: Selector("textField:completions:forPartialWordRange:indexOfSelectedItem:")){
